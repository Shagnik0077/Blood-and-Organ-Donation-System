<?php
    ...
    if($_SERVER["REQUEST_METHOD"] == "POST" && $_REQUEST['action']=="submit")
    {
        ...
        $insertQuery = "INSERT INTO t_blood_doner_pending_table VALUES ('', '$name','$email','$type', '$age','$bloodGroup','$donationPlace','$selectedPlace','$status','S')";
...
<body>
  ...
  <div class="container">
        <h1>Donate Blood</h1>
        <form id="bloodDonationForm" method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>">
            <!-- Your form fields -->
            ...
            <div class="form-group">
                <label>Donation Place:</label><br>
                <label>
                    <input type="radio" id="selectPlace" name="donationPlace" value="hospital" required>
                    Hospital
                </label><br>
                <label>
                    <input type="radio" id="selectPlace" name="donationPlace" value="bloodBank" required>
                    Blood Bank
                </label>
            </div>
