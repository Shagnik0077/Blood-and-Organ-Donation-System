...
<head>
  ...
  <title>Blood Recipent Request</title>
  <style>
    body {
...
<div class="dashboard-container">
    <h2 class="Blood_Bank_Details ">Blood Recipent Request</strong></h2>
   ...
   <?php
      $query = "SELECT * FROM t_blood_recipent_pending_table WHERE status = 'Pending' ORDER BY id ASC ";
      ...
      while ($row = mysqli_fetch_array($result))
      {
        ...
        echo '<td>';
        echo '<form action="admin_blood_recipent_request.php" method="post">';
...
<?php
       ...
       {
         $id = $_POST['id'];
         $select = "UPDATE t_blood_recipent_pending_table SET status = 'Approved' WHERE id = '$id'";
        $result = mysqli_query($DB_CONNECT, $select);
         $select1 = "UPDATE t_blood_recipent_pending_table SET process = 'ND' WHERE id = '$id' ";
         $result1 = mysqli_query($DB_CONNECT, $select1);
       
         $query1 = "SELECT * FROM t_blood_recipent_pending_table WHERE status = 'Approved' AND process='ND' AND id='$id' ORDER BY id ASC ";
         ...
         while($row=mysqli_fetch_array($result2))
          {
           ...
           if (mysqli_num_rows($resultCheck) > 0) {
               ...
               $select = "UPDATE t_blood_recipent_pending_table SET process='D' WHERE id='$id'";
         $result = mysqli_query($DB_CONNECT, $select);
       }
        ...
        if (mysqli_num_rows($resultCheck) > 0) {
            ...
            $currentQuantity = $row2[$bloodGroup];
              if($currentQuantity <= 0){
                ...
                mysqli_query($DB_CONNECT,$sqlUpdate);
                echo "Quantity for Blood Group $bloodGroup reduced by 1 at $placename.";
                header("location:admin_blood_recipent_request.php");
              }
            }   
        else{
             echo "Hospital or Blood Group not found.";
