...
<head>
  ...
  <title>Organ Recipient Request</title>
  <style>
body {
...
<body>
  ...
  <div class="dashboard-container">
    <h2 class="Blood_Bank_Details">Organ Recipient Request</h2>
    ...
    <table align="center">
      ...
      <?php
      $query = "SELECT * FROM t_organ_recipient_pending_table WHERE status = 'Pending' ORDER BY id ASC ";
      ...
      while ($row = mysqli_fetch_array($result))
      {
        ...
        echo '<td>';
        echo '<form action="admin_organ_recipent_request.php" method="post">';
...
{
  $id = $_POST['id'];
  $select = "UPDATE t_organ_recipient_pending_table SET status = 'Approved' WHERE id = '$id'";
  $result = mysqli_query($DB_CONNECT, $select);
  $select1 = "UPDATE t_organ_recipient_pending_table SET process = 'ND' WHERE id = '$id' ";
  $result1 = mysqli_query($DB_CONNECT, $select1);

  $query1 = "SELECT * FROM t_organ_recipient_pending_table WHERE status = 'Approved' AND process='ND' AND id='$id' ORDER BY id ASC ";
    ...
    if (mysqli_num_rows($resultCheck) > 0) {
        ...
        $currentQuantity = $row2[$bloodGroup];
          if($currentQuantity <= 0){
            echo "The quantity for Hearts of $bloodGroup at $hospital is already 0 or less.";
            } 
          else{
            ...
            echo "Quantity for Blood Group $bloodGroup reduced by 1 at $hospital.";
        ...
        $select = "UPDATE t_organ_recipient_pending_table SET process='D' WHERE id='$id'";
  $result = mysqli_query($DB_CONNECT, $select);
...
{
  $id = $_POST['id'];
  $select = "DELETE FROM t_organ_recipient_pending_table WHERE id = '$id'";
  $result = mysqli_query($DB_CONNECT, $select);
